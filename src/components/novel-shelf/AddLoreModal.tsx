...
import { X, ScrollText, Globe, StickyNote, Tag } from 'lucide-react';
...
export function AddLoreModal() {
  ...
  const handleSubmit = async () => {
    if (!title.trim()) { toast.error('Judul lore wajib diisi'); return; }
    if (!content.trim()) { toast.error('Konten lore wajib diisi'); return; }
    setSaving(true);
    try {
      const tags = tagsInput.split(',').map((t) => t.trim()).filter(Boolean);
      await loreApi.create(selectedNovelId!, {
        title: title.trim(),
      ...
      toast.error(err instanceof Error ? err.message : 'Gagal menambah lore');
    } finally {
      setSaving(false);
    }
  };
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  {CATEGORIES.map((cat) => (
                    <button key={cat.id} onClick={() => setCategory(cat.id)} className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-xs font-medium transition-colors ${category === cat.id ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
              ...
              <div>
                ...
                <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Contoh: Asal Usul Kerajaan Aethon" className="bg-background" />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 block">Konten *</label>
                <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Tulis lore di sini..." rows={8} className="w-full px-3 py-3 rounded-lg bg-background border border-border text-sm resize-none focus:outline-none focus:ring-2 focus:ring-ring leading-relaxed" />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 flex items-center gap-1"><Tag className="w-3 h-3" /> Tags (pisahkan dengan koma)</label>
                <Input value={tagsInput} onChange={(e) => setTagsInput(e.target.value)} placeholder="Contoh: kerajaan, sejarah, politik" className="bg-background" />
              </div>
              <Button onClick={handleSubmit} className="w-full" size="lg" disabled={saving}>{saving ? 'Menyimpan...' : 'Simpan Lore'}</Button>
            </div>
          </motion.div>
        </motion.div>
      )}
